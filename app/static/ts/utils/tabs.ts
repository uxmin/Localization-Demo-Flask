document.addEventListener("DOMContentLoaded", () => {
  const pannel = (document.querySelector('div[name="pannel"]') as HTMLDivElement) || null;
  if (!pannel) return;
  ...
  tabs.forEach((tab) => {
    tab.addEventListener("click", function (this: HTMLLIElement) {
      ...
      tabs.forEach((t) => {
        const tabEl = t as HTMLLIElement;
        if (tabEl === this) {
